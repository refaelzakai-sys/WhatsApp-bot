...
// יצירת קבצי בסיס בצורה בטוחה כדי למנוע קריסת "סטטוס 1"
try {
    ...
    console.error("שגיאה ביצירת קבצים:", err);
}
...
// מאזין לכל כתובות ה-IP, דרישה של חלק מהשרתים בענן
...
async function startBot() {
    try {
        ...
        const sock = makeWASocket({
            ...
            logger: pino({ level: 'silent' }), // משתיק שגיאות מיותרות שיכולות להקריס את הלוגים
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    setTimeout(startBot, 3000); // ניסיון חיבור מחדש אחרי 3 שניות
                }
        ...
        sock.ev.on('messages.upsert', async (m) => {
            try {
                ...
                // קריאה בטוחה של אנשי קשר
                let savedContacts = [];
                try {
                    const fileData = fs.readFileSync(CONTACTS_FILE, 'utf8');
                    if (fileData) savedContacts = JSON.parse(fileData);
                } catch (readErr) {
                    savedContacts = [];
                }
                ...
                console.error("שגיאה בטיפול בהודעה:", msgErr);
            }
        });
    } catch (botErr) {
        console.error("שגיאה קריטית בהפעלת הבוט:", botErr);
    }
}
